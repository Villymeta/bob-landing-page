...
export default function ShopPage() {
  ...
  return (
    <main className="min-h-screen bg-black text-white px-6 py-10">
      <h1 className="text-5xl font-extrabold text-center mb-12 tracking-wide text-bobyellow drop-shadow-lg">
        B.O.B Collection
      </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-7xl mx-auto">
        {defaultProducts.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className={`relative rounded-2xl p-6 flex flex-col items-center border transition-all duration-300 
              ${
                isSoldOut
                  ? "border-gray-700 opacity-50"
                  : "border-bobyellow hover:shadow-[0_0_25px_rgb(248,228,159,0.4)]"
              } bg-gradient-to-b from-zinc-950 to-neutral-900`}
            >
              {/* Product Image */}
              <div className="relative">
                ...
                />
                {isSoldOut && (
                  <span className="absolute top-2 left-2 bg-bobyellow text-black text-xs font-extrabold px-3 py-1 rounded-md shadow-md uppercase">
                    Sold Out
                  </span>
                )}
              </div>
              ...
              {/* Info */}
              <h2 className="text-xl font-semibold tracking-wide text-bobyellow">
                {product.name}
              </h2>
              ...
              {product.sizes && (
                <div className="w-full mb-3">
                  ...
                  <select
                    className="w-full bg-black border border-gray-700 text-white rounded-lg px-3 py-2 focus:border-bobyellow focus:ring-1 focus:ring-bobyellow transition"
                    ...
                    {product.sizes.map((size) => {
                      ...
                      return (
                        <option key={size} value={size} disabled={available === 0}>
                          {size} {available === 0 ? "(Sold Out)" : `(In stock: ${available})`}
                        </option>
                      );
                    })}
                  </select>
              ...
              <div className="w-full">
                ...
                />
                {product.id === "BOB Beanie" && (
                  <p className="text-xs text-bobyellow mt-1 italic">Limit 2 per customer</p>
                )}
              </div>
              ...
              <button
                ...
                disabled={isSoldOut}
                className={`mt-5 w-full text-black font-extrabold rounded-xl py-3 uppercase tracking-wider transition-all duration-300 ${
                  isSoldOut
                    ? "bg-gray-500 cursor-not-allowed"
                    : "bg-bobyellow hover:brightness-110 active:scale-95"
                }`}
              >
